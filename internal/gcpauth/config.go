// Package gcpauth provides GCP Workload Identity Federation authentication.
package gcpauth
...
// Config represents GCP external account credential configuration for Workload Identity Federation.
...
// CredentialSource specifies where to obtain the subject token from.
...
// CredentialFormat specifies the format of the credential source.
type CredentialFormat struct {
	Type                  string `json:"type,omitempty"`
	SubjectTokenFieldName string `json:"subject_token_field_name,omitempty"`
}

// LoadConfig reads and validates a GCP WIF config file.
...
// IsAWS returns true if the config uses AWS IMDS as credential source.
